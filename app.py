# app.py
# CSA S16 - Beam Flexure Calculator
#
...
def require_disclaimer_acceptance() -> None:
    st.set_page_config(page_title="Beam Flexure - CSA S16",
...
md("<h2 style='margin-bottom:0'>CSA S16 &mdash; Beam Flexure Design</h2>")
note("Factored moment resistance per Clauses 11, 13.5 and 13.6, with the "
     "shear check of Cl. 13.4.1.1 and the serviceability limits of "
     "Table D.1. Every check states its clause, then shows the work.")
...
_PROP_ROWS = [
    ("Geometry", "d", geom["d"], "mm", 1,
     "Overall depth. Sets h and the elastic depth d/2."),
    ("Geometry", "b", geom["b"], "mm", 1,
     "Flange width. Cl. 11.3.1 c) takes b/2 for an I-shape flange."),
    ("Geometry", "t", geom["t"], "mm", 1,
     "Flange thickness. Denominator of b_el/t."),
    ("Geometry", "w", geom["w"], "mm", 1,
     "Web thickness. Denominator of h/w and the shear area."),
    ("Geometry", "h", geom["h"], "mm", 1,
     "Clear web depth, d - 2t, per Cl. 11.3.2 d)."),
    ("Geometry", "A", _area, "mm2", 0, "Gross area."),
    ("Flexure", "Zx", _f(shape.get("Zx")), "10^3 mm3", 0,
     "Plastic modulus. Mr = phi Zx Fy for Class 1 and 2, Cl. 13.5 a)."),
    ("Flexure", "Sx", _f(shape.get("Sx")), "10^3 mm3", 0,
     "Elastic modulus. Mr = phi Sx Fy for Class 3, Cl. 13.5 b)."),
    ("Flexure", "Ix", _f(shape.get("Ix")), "10^6 mm4", 1,
     "Strong-axis second moment of area. Drives the deflection check."),
    ("Lateral torsional buckling", "Iy", _f(shape.get("Iy")), "10^6 mm4", 1,
     "Weak-axis second moment of area. First term of Cl. 13.6 Eq. 1."),
    ("Lateral torsional buckling", "J", _f(shape.get("J")), "10^3 mm4", 1,
     "St. Venant torsion constant. The GJ term of Cl. 13.6 Eq. 1."),
    ("Lateral torsional buckling", "Cw", _f(shape.get("Cw")), "10^9 mm6", 1,
     "Warping constant. The warping term of Cl. 13.6 Eq. 1."),
]

with st.expander("Section properties  -  " + str(selected_section),
                 expanded=False):
    ...
    for _grp, _sym, _val, _unit, _nd, _why in _PROP_ROWS:
        if _val is None:
            continue
        if _grp != _cur:
            _cur = _grp
            _html.append("<tr><td colspan='3' style='padding:10px 0 3px 0;"
                         "font-weight:600'>" + _cur + "</td></tr>")
        _u = (" " + _unit) if _unit else ""
        _u = (_u.replace("mm2", "mm<sup>2</sup>")
              .replace("mm3", "mm<sup>3</sup>")
              .replace("mm4", "mm<sup>4</sup>")
              .replace("mm6", "mm<sup>6</sup>")
              .replace("10^3", "10<sup>3</sup>")
              .replace("10^6", "10<sup>6</sup>")
              .replace("10^9", "10<sup>9</sup>"))
        _html.append("<tr>"
                     "<td style='padding:3px 16px 3px 14px;"
                     "white-space:nowrap'>" + _sym + "</td>"
                     "<td style='padding:3px 16px 3px 0;"
                     "white-space:nowrap'>" + num(_val, _nd) + _u + "</td>"
                     "<td style='padding:3px 0;color:#8b949e;"
                     "font-size:0.88em'>" + _why + "</td></tr>")
    _html.append("</table>")
    md("".join(_html))
    st.caption("Source: CISC Structural Section Tables SST12.1, read at run "
               "time through sst12.py.")

with st.expander("Raw table record  -  " + str(selected_section),
                 expanded=False):
    _canonical_keys = ("designation", "d", "b", "t", "w", "k", "Ix", "Sx",
                       "Zx", "rx", "Iy", "Sy", "Zy", "ry", "J", "Cw")
    _canon_rows = [(k, shape.get(k)) for k in _canonical_keys
                   if shape.get(k) is not None]
    if _canon_rows:
        st.markdown("**Canonical properties (consumed by the engine)**")
        st.table(pd.DataFrame(_canon_rows,
                              columns=["Property", "Value"]
                              ).set_index("Property"))
    _raw_rows = [(k, v) for k, v in shape.items()
                 if k not in _canonical_keys]
    if _raw_rows:
        st.markdown("**All other workbook columns**")
        st.table(pd.DataFrame(_raw_rows, columns=["Column", "Value"]
                              ).set_index("Column"))
