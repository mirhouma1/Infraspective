...
def vr_groove_N(Am_mm2: float, Aw_mm2: float, Fu: float, Xu: float) -> dict:
    ...
    return {
        "Vr_base_N": Vr_base,
        "Vr_base_kN": Vr_base / 1000,
        "Vr_weld_N": Vr_weld,
        "Vr_weld_kN": Vr_weld / 1000,
        "Vr_N": gov,
        "Vr_kN": gov / 1000,
...
def tr_pjp_combined_N(
    An_mm2: float, Aw_mm2: float, Fu: float, Xu: float, Ag_mm2: float, Fy: float
) -> dict:
...
with mat2:
    Fy = st.number_input(
        "Fy (MPa)", min_value=200.0, value=float(Fy_default), step=5.0, key="Fy"
    )
    Fu = st.number_input(
        "Fu (MPa)", min_value=200.0, value=float(Fu_default), step=5.0, key="Fu"
    )
...
if weld_type == "Fillet Weld":
    ...
    with g2:
        check_base = st.checkbox(
            "Check base metal (over-matched electrodes)?", key="chk_base"
        )
        if check_base:
            Am_mm2 = st.number_input(
                "Am — fusion face shear area (mm2)",
                min_value=0.0,
                value=1000.0,
                step=50.0,
                key="Am_fillet",
    ...
    with g1:
        Am_mm2_groove = st.number_input(
            "Am — shear area of fusion face (mm2)",
            min_value=0.0,
            value=2000.0,
            step=50.0,
            key="Am_groove",
        ...
        Aw_mm2 = st.number_input(
            "Aw — effective weld throat area (mm2)",
            min_value=0.0,
            value=2000.0,
            step=50.0,
            key="Aw_groove",
    ...
    with g2:
        if "Partial" in weld_type:
            ...
            An_mm2 = st.number_input(
                "An — nominal fusion face area (mm2)",
                min_value=0.0,
                value=2000.0,
                step=50.0,
                key="An",
...
with det1:
    t_thinner = st.number_input(
        "t_thinner — thinner part joined (mm)",
        min_value=1.0,
        value=10.0,
        step=1.0,
        key="t_thin",
    ...
    t_thicker = st.number_input(
        "t_thicker — thicker part joined (mm)",
        min_value=1.0,
        value=12.0,
        step=1.0,
        key="t_thick",
...
with res_col:
    ...
    if weld_type == "Fillet Weld":
        ...
        with st.expander("📐 Show calculation steps", expanded=True):
            ...
            for i, s in enumerate(segments):
                ...
                throat_rows.append(
                    f"- Seg {i + 1} — Substitute: Aw = {throat:.3f} × {s['L']:.0f}"
                )
        ...
        with st.expander("📐 Show calculation steps", expanded=True):
            ...
            if len(segments) == 1:
                ...
                mw_rows.append(
                    f"- θ₂ = {theta_max:.0f}° (Seg {idx_max + 1}, nearest to 90°)"
                )
                for i, s in enumerate(segments):
                    num = 0.85 + s["theta"] / 600
                    denom = 0.85 + theta_max / 600
                    ...
                    mw_rows.append(
                        f"- Seg {i + 1} — Result: **Mw = {mw_values[i]:.4f}**"
                    )
            st.markdown("\n".join(mw_rows))
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if weld_type == "Complete Joint Penetration (CJP) Groove":
            ...
            st.markdown(
                "#### Step 2 — Tension Resistance  *(Cl. 13.13.3.2 / 13.13.3.3)*"
            )
            if not has_fillet:
